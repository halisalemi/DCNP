...
vector<long> solveDCNP_thin_formulation(KGraph &g, long k, long b, vector<long> &Heuristic_sol)
{
	...
	try
	{
		...
		vector<long> NonCriticalNodes = Preprocessing(g);
		...
		cerr << "Number of minimal length-2 connecter inequalities is " << length_2_connectors << endl;
		...
		//	}
		//}
		//cerr << "Number of minimal length-3 connecter inequalities is " << length_3_connectors << endl;
		...
		cout << "Time spent in callback is: " << integer_separation::TotalCallbackTime << endl;
...
vector<long> solveDCNP_thin_formulation_fractional(KGraph &g, long k, long b, vector<long> &Heuristic_sol)
{
	...
	try
	{
		...
		cerr << "Time spent in integer part of callback is: " << fractional_separation::TotalCallbackTimeInteger << endl;
		cerr << "Time spent in fractional part of callback is: " << fractional_separation::TotalCallbackTimeFractional << endl;
...
/*Based on Algorithm 1 in paper "A faster algorithm for betweenness centerality",
...
vector<long> FindTopTBetweenessCentralityNodes(KGraph &g, long T)
{
	...
	vector<long> TopT_BC;
	priority_queue<std::pair<double, long>> q;
	for (long i = 0; i < C_B.size(); i++)
	{
		q.push(std::pair<double, long>(C_B[i], i));
	}
...
vector<long> DCNP_Heuristic(KGraph &g, long k, long b)
{
	...
	vector<long> D = FindTopTBetweenessCentralityNodes(g, 2*b);
	vector<long> D_Star;
	float q_star = INFINITY;
	vector<long> D_copy = D; //D_copy is just a copy of D
	long delta_b = b / 2;
	long NumOfClosePairs = 0;

	while (D.size() > b - delta_b)
	{
		if (D.size() == b && NumOfClosePairs <= q_star)
		{
			D_Star = D;
